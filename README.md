...
以管理员身份运行 `scripts\schedule_task.bat`，脚本会自动创建每天上午9点执行的定时任务。
...
例如，改为每天下午2点：
```bash
schtasks /Change /TN "AI_News_Feed" /ST 14:00
```
...
1. 打开"任务计划程序"（在开始菜单搜索）
2. 创建基本任务
3. 设置触发器为"每天"，时间设置为 09:00
4. 操作选择"启动程序"
5. 程序或脚本：`python`
6. 添加参数：`D:\IndustryNewsFeed\main.py`（请改为您的实际路径）
7. 起始于：`D:\IndustryNewsFeed`（请改为您的实际路径）
...
### 定时任务未执行
- 确认任务已创建并启用（任务名称：`AI_News_Feed`）
- 检查Python路径是否正确
- 确认任务计划程序中的任务状态为"就绪"
- 查看任务计划程序中的执行历史，检查是否有错误信息
- 如果任务执行失败，检查日志文件 `news_feed.log` 了解详细错误
